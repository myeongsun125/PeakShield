...
// static/js/process_flow_team.js
// 팀원 공정 흐름 탭 로직을 베이스에 안전하게 이식한 버전
// - 전기료/CO2 탭에는 영향 없음
// - 실제 데이터는 window.INITIAL_STAGES에 넣어주면 자동 반영

window.INITIAL_STAGES = window.INITIAL_STAGES || [];
...
function initClock() {
    setInterval(() => {
        const now = new Date();
        const el=document.getElementById('header-time'); if(el) el.innerText = now.toLocaleTimeString();
    }, 1000);
}
...
function addLog(stage, message, type = 'info') {
    ...
    logCount++;
    const cnt=document.getElementById('log-count'); if(cnt) cnt.innerText = `${logCount} EVENTS RECORDED`;
